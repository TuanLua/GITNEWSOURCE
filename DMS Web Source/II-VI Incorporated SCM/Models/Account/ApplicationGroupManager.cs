...
namespace II_VI_Incorporated_SCM.Models.Account
{
    public class ApplicationGroupManager
    {
        ...
        public async Task<IdentityResult> RefreshUserGroupRolesAsync(string userId)
        {
            ...
            var addTheseRoles = allRoles.Where(r => newGroupRoles.Any(gr => gr.ApplicationRoleId == r.Id));
            //var roleNames = addTheseRoles

            //// Add the user to the proper roles
            //await _userManager.AddToRolesAsync(userId, roleNames);

            // custom by Trung Vu
            var arrRole = addTheseRoles.Select(x => x.Id).ToArray();
            foreach (var role in arrRole)
            {
                user.Roles.Add(new ApplicationUserRole { RoleId = role, UserId = userId });
            }
            await _db.SaveChangesAsync();
            return IdentityResult.Success;
        }
        ...
        public IdentityResult SetUserRole(string id, params string[] arrRoles)
        {
            RefreshUserGroupRoles(id);
            return IdentityResult.Success;
        }
